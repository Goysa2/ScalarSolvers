...
function TR_generic_Stop(h :: AbstractNLPModel,
    ...
    t = h.meta.x0[1]; iter = 0; # Establish starting point and iteration counter.
    fₖ = obj(h, [t])[1]
    f₀ = copy(fₖ)
    gₖ = grad(h, [t])[1]
    g₀ = copy(gₖ)
    ...
    OK = update_and_start!(nlpstop, x = t₀, fx = fₖ, )
    OK = update_and_start!(nlpstop, x = xt, fx = ft, gx = ∇f, g0 = copy(∇f))
